...
- **18/09/2026 - Pipeline de Mídia por EXIF (indexação do acervo + cruzamento com atividades):**
    - **Ferramentas criadas:** `tools/indexar_midia_exif.py` (indexa data de captura, GPS, modelo de câmera, dimensões e **categoria de origem** por arquivo) e `tools/cruzar_midia_atividades.py` (cruza as fotos com `eventos_marcados` por janela de data, marcando ambiguidades em vez de adivinhar). Ambas somente-leitura sobre o acervo.
    - **Indexação:** **57.921 arquivos de mídia** processados em `E:\06_Backup_Local\POCO_PADF\`. Categorias: 38.141 caches de app (`device\Pictures\`), 14.030 miniaturas de vídeo (`.thumbnails`), 4.851 imagens de redes sociais, 363 screenshots, **512 fotos de câmera** e 24 downloads/scans.
    - **Achado crítico — a data de arquivo não serve:** apenas **944 mídias (1,6%) têm EXIF real** com `DateTimeOriginal`. As outras 56.977 só têm `mtime`, que é a **data da cópia**. Exemplo do risco: `DCIM\Camera` tem `mtime` de 06 e 16/10/2024, mas EXIF de 13 e 25/06/2024. Um agrupamento inicial de "617 fotos em 26/02/2026" revelou-se um lote de miniaturas de vídeo copiadas naquele dia. **Decisão de projeto:** o critério de confiança do cruzamento passou a ser `fonte_data == 'exif'`, não a pasta nem o nome.
    - **Equipamentos identificados:** `21061110AG` (POCO M3 Pro 5G, 783 fotos), `Mi A3` (125), `NIKON D800`/`D5100`/`SM-M225FV` (6).
    - **Cruzamento:** 944 fotos com data confiável em 252 dias → **21 atividades com fotos candidatas**, sendo **8 sem ambiguidade** e **13 marcadas como ambíguas** para revisão humana (ex.: as turmas I e II do Curso de DJ ocorreram nos mesmos dias e compartilham as mesmas 80 fotos; o Curso de Fotografia SENAI se sobrepõe a seis outras atividades).
    - **Impacto direto na cobertura:** **19 das 42 lacunas agora têm fotos reais**, incluindo prioridades ALTAS — `71` CONARH 2026 (5 fotos), `21` FESPA 2025 (24), `19` FEBRATÊXTIL (23), `35` Automaquiagem (17), `30` ALPHAGRAPHICS 2026 (15), `9` ABF EXPO 2024 (12), `33` EXPOPRINT 2026 (9), `36` FEBRATÊXTIL 2026 (5) e `34` DOWNLANDIA no SBT (4).
    - **Limites registrados:** o reconhecimento facial **ainda não rodou** nas atividades pendentes (as 365 linhas de `fotos_reconhecidas` cobrem só eventos antigos, 2017-2024), então a pauta chega ao redator com `atendentes_presentes: 0` nas lacunas de 2025-2026; o enrollment biométrico tem apenas **39 dos 112 candidatos**; os 8.278 vídeos não entram por EXIF (precisam de `QuickTime CreateDate`, em trilha própria).
    - **Recomendação operacional:** usar `robocopy /COPY:DAT` no próximo offload do celular, para que o `mtime` deixe de ser inútil na preservação de datas.
    - **Saídas:** `RELATORIO_MIDIA_EXIF_ATIVIDADES_18SET2026.md`; índice em `apoio/midia_exif_poco_padf.csv` e cruzamento em `apoio/midia_por_atividade.csv` (não versionados por tamanho). `.gitignore` passou a versionar a pasta `tools/`.
    - **Refinamento no mesmo dia (salto de 50× na base utilizável):** o extrator inicial só reconhecia nomes com hora (`IMG_20240613_164329.jpg`) e descartava o padrão do WhatsApp (`IMG-20250325-WA0004.jpg`), que responde por **37.972 arquivos** em `MIDIA\imagens\<ano>\<mês>\`. Após adicionar os padrões de data sem hora, de nome com separadores (`share_2024-06-27_04_36_33_588`) e de pasta `AAAA\MM`, a indexação passou a **62.963 arquivos**, com **47.946 fotos de data utilizável** (era 944) e 1.036 com EXIF real de câmera.
    - **Controle de qualidade adicionado:** o cruzamento ganhou métricas de **concentração** (dias distintos, dia de pico e % no pico). Constatação importante: **volume não é precisão** — nenhuma atividade passou de 40% de concentração, e o Curso de Fotografia SENAI acumulou 3.096 fotos diluídas em 96 dias (5,7% no pico), ou seja, varredura de WhatsApp do período e não cobertura do curso. O índice serve para **localizar candidatos**, não para montar galeria automaticamente.
    - **Integração com a esteira:** `tools/gerar_midia_json.py` publica o cruzamento em `/home/projetoame/midia_por_atividade.json` (fora do web root) e o `api_ame_pautas.php` passou a anexar `midia_acervo` a cada pauta — com contagem, período e a ressalva de curadoria. Verificado: as 8 primeiras pautas pendentes já retornam evidência (CONARH 2026 = 145 fotos, DOWNLÂNDIA no SBT = 172, Automaquiagem = 705).

- **18/09/2026 - Auditoria da Esteira Noturna NASANET (prospecção parada) e Checagem da AIsa.one:**
    - **Contexto:** verificação de se os 245 registros da tabela local `eventos` estariam fora da esteira do agente noturno.
    - **Resposta: NÃO estão fora — não há nada a injetar.** Comparação nome a nome: **0 eventos da base local ausentes em produção**. A produção tem **965 eventos distintos** (981 linhas, 959 nomes únicos) contra 241 distintos na cópia local — ou seja, a local é um recorte antigo e menor. Os 245 locais já são insumo da esteira.
    - **Onde a esteira roda:** VPS1, via `cron` → `/home/novaeratec/automacao/shift_scheduler.py --tick` (a cada minuto, das 20h às 6h). Ela orquestra **5 turnos**: 20h BiometriaAgent · 22h **EventosAgent** (`eventos_agent_miner.py`) · 00h GraficasAgent · 02h PousadasAgent · 04h PHPConvertAgent. O `eventos_agent_miner.py` opera justamente `projetoame.eventos` (Locais → Eventos → Expositores → Contatos → Enriquecimento IA → Injeção CRM), com `BATCH_SIZE=30`.
    - **Achado crítico — a esteira roda mas colhe zero.** O ciclo de 17/09 marca os 5 turnos como `completed` com **`cota_atual: 0` e `tempo_decorrido_minutos: 1`**. Nos logs: o EventosAgent processou 60 candidatos e **todos falharam** com `HTTP 401 — Invalid proxy server token... Unable to find token in cache or LiteLLM_VerificationTokenTable`; encerrou com *"Leads novos qualificados para o Projeto AME CRM: 0"*. O PousadasAgent falhou pelo mesmo 401 (*0 pousadas, 0 trilhas*). O GraficasAgent sofreu `Read timed out` no `llm.netmailing.com.br` e não extraiu nada. O PHPConvertAgent não tinha fila. **Causa raiz: a virtual key `sk-...jPdQ` usada em `/home/novaeratec/automacao/.env` foi revogada e não existe mais no LiteLLM** — e não consta no `SECRETS.md`, sendo uma chave antiga e não documentada.
    - **Segundo achado — o turno de biometria é um no-op.** O `BiometriaAgent` (20h-22h) está cadastrado com `script_comando: "none"`. Isso explica por que `fotos_reconhecidas` não tem nada posterior a 2024: **o agente que deveria rodar o reconhecimento facial nunca foi conectado a um script**.
    - **Diagnóstico de chaves (testado da própria VPS1, com User-Agent de navegador):** `automacao/.env` → **401** (revogada). `agenet` → **200** em `hermes-local` (escopo correto: `hermes-local`, `triagem-whatsapp`). `novaeratec` → 200 em `text-chat`, mas **400 "Provider returned error"** em `hermes-local`. `projetoame` → 400 nos dois. Observação de método: requisição sem User-Agent recebe **403 do Cloudflare (erro 1010)** em `llm.netmailing.com.br`, o que mascara o resultado real — inclusive de chaves válidas.
    - **AIsa.one — saldo ocioso.** Levantado que **nenhum script de produção referencia a AIsa.one**: não há `AISA_API_KEY` em nenhum `.env` (VPS1, VPS2) e a única menção no workspace é um log de WhatsApp. Ou seja, o saldo está disponível mas **nada o consome**; a integração segue no estágio de "teste piloto" descrito em `apoio/2026-09-11`. Para ativá-la é necessária a chave de API do dashboard.
    - **Degradação simultânea da camada de IA (achado colateral relevante).** Ao tentar ler o print do dashboard da AIsa.one, três caminhos de visão falharam e expuseram o estado real dos pools: (1) **o OpenRouter está sem crédito** — o erro literal é *"This request requires more credits... You requested up to 3000 tokens, but can only afford 172"*, depois *"Prompt tokens limit exceeded: 1622 > 215"*; é a mesma causa do HTTP 402 que o Content Factory já apresentava nos pools `copy-expert`/`paid-fallback`; (2) o pool `vision-ocr` roteia para `grok-2-vision-1212`, ao qual a equipe **perdeu acesso** (404); (3) o `gemini-free-fallback` está em **rate limit/cooldown** (429). O modelo local `qwen3-vl:30b` funciona, mas levou 249s só para carregar os 19,6 GB e não concluiu a inferência em 25 minutos. **Conclusão operacional: hoje não há caminho de visão confiável no ecossistema, e a esteira editorial depende de pools degradados.**
    - **Direitos de imagem — levantamento:** existem **41 termos assinados** em produção (`docs/direito-de-imagem-de-<nome>.pdf`), índice legado em `docs/autorizacoes.csv` e **156 registros para 83 candidatos** na tabela `documentos`. A ocultação por foto **já funciona** (`fotos_reconhecidas.oculta` + `api_ocultar_foto.php`, visível em `/meuperfil`). O que falta é o opt-out **global**, a **página pública** e — o mais importante — o **enforcamento do consentimento na esteira de publicação**, hoje inexistente. Desenho completo em `PLANO_DIREITOS_DE_IMAGEM_AME.md`.
    - **Correção de método registrada:** uma primeira contagem indicou "0 autorizações em produção" por eu ter buscado `direito_de_imagem` (underscore) quando os arquivos usam `direito-de-imagem` (hífen). O número correto é 41.
