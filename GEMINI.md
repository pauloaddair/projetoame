...
- **Esteira de cobertura:** plano completo em **`PLANO_ESTEIRA_COBERTURA_BLOG_AME.md`**. Causa raiz dos artigos genéricos: o pool `TENANT_CURATED_PAUTAS["projetoame"]` do motor tinha **só 2 pautas editoriais genéricas** (que geraram exatamente os posts de 12 e 15/09/2026); com o pool esgotado, a próxima pauta cairia no RSS de tecnologia do G1.
- **`link_artigo` corrigido (18/09/2026):** 15 vínculos regravados por ID. O defeito vinha do `database/add_link_artigo.php`, que casava pela primeira palavra do nome do evento — por isso o post do Curso de DJ aparecia em 4 atividades. **Contador de cobertura: `57 - COUNT(link_artigo <> '')` = 42 pendentes.** O campo é renderizado publicamente em `pages/eventopublico.php`.
- **Endpoint de pautas (novo):** `include/api_ame_pautas.php` — `GET` lista atividades realizadas sem artigo (com local, empresas contratantes, atendentes e fotos); `POST` faz o write-back do `link_artigo`. Autenticação por header **`X-AME-Token`** (segredo em `/home/projetoame/ame_pautas.token`, **fora do web root**; sem o arquivo responde 503).
- **Esteira em produção:** o tenant `projetoame` no Content Factory está com `pauta_source: "activities"` e `strict_scope: true`, consumindo as atividades via `ame_pautas_provider.py` (repo NASANET, `infra/`). O provedor só é acionado nesse tenant — os outros 14 canais seguem inalterados. Dry-run na VPS2 aprovou artigo ancorado com 95 pts.
