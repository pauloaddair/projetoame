...
$querycandidatos = "SELECT nome,Email FROM candidatos ORDER BY nome;";
$candidatos = mysqli_query($conexao,$querycandidatos);
...
if ($_SERVER['REQUEST_METHOD']=="POST"){
	...
	$query = "SELECT candidatos.* FROM candidatos,usuarios WHERE candidatos.Email LIKE '".$email."'";
	echo $query . "<br>";
...
<body>
	<div class="container">
		...
		<form method="post">
			<input type="text" id="salvar" name="salvar" value="1">
			<div class="row mt-1">
				<div class='col-12'>
					<?
						echo $msg;
		...
		<div class="col col-md-6 align-items-stretch d-flex mb-2">
			...
			<?
				$id = $row0['id'];
				$query1 = "SELECT horarios.*,expositores2024.empresa FROM horarios,expositores2024 
				WHERE horarios.evento_id = ".$id. " 
				AND horarios.empresa_id = expositores2024.expositor_id
				...
				while ($row0 = mysqli_fetch_assoc($horarios)){
					...
					<label class="custom-control-label" for="<? echo digitos($row0['horario_id'])?>"><? echo date("d-M-Y",strtotime($row0['data_inicio'])). " (<strong>".$row0['empresa']."</strong>) - ". $row0['tipo']?><br>
						<? 
			...
			<div class="row justify-content-center">
				...
				<div class="md-form">
					...
					<?
						while ($cand = mysqli_fetch_assoc($candidatos)){
						?>
						<option value="<? echo $cand['Email']?>"><? echo $cand['nome']?></option>
						<?
						}	
					?>
					</select>
