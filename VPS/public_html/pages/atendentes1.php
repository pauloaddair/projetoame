<?php
	...
	$dados = array(
		...
		array("arquivo","","","file","far fa-file"),
...
//		echo "<p>Incremento=".$salvar."</p><p>Candidato_id=".$candidato_id."</p>";
	...
	?>
		...
		If (mysqli_num_rows($r)>0){
			...
			if ($salvar==1){
				...
				if (!empty($tnpFile)){
	//				$img_id = 0;
					$nomearquivo = "docs/". $_FILES['file']['name'] ;
					$diretorio = before("pages",__DIR__);
	//					exit;
					$file_parts = pathinfo($diretorio ."/". $nomearquivo);
	//					echo $file_parts . "<br>";
					$icone = $nomearquivo;
	//				echo "<br>". $nomearquivo;
/*
					if ($file_parts['extension']=='pdf'){
						$icone = "img/card-2x1.png";
						}
*/
		...
		<div class="col col-md-6 align-items-stretch d-flex mb-2">
			...
			<div class="row justify-content-center">
				...
				<div class="md-form">
					...
					<?
						} elseif ($dados[$i][3]=="file") {
					?>
							<div class="md-form">
								<i class="<? echo $dados[$i][4]?> prefix grey-text"></i>
								<input type="<? echo $dados[$i][3]?>" id="<? echo $dados[$i][0]?>" name="<? echo $dados[$i][0]?>" class="form-control" placeholder="<? echo $dados[$i][2]?>">
								<label for="<? echo $dados[$i][0]?>"><? echo $dados[$i][1]?></label>
							</div>
					<?
