<?php
    ...
    $query = 'SELECT nome FROM convidados WHERE nome LIKE :term%'; // select query
    ...
    if ( $stmt->rowCount()>0 ) {
        ...
        while($recResult = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $json_row["id"] = $recResult['convidado_id'];
            $json_row["value"] = $recResult['convidado_id'];
            $json_row["label"] = $recResult['nome'];
        ...
        $json_row["id"] = "#";
        $json_row["value"] = "";
        $json_row["label"] = "Nada encontrado!";
        array_push($sugg_json, $json_row);
    }
