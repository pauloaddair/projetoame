...
class ICS {
    var $data;
    var $name;
    function ICS($start,$end,$arquivo,$name,$description,$location) {
        $this->name = $arquivo;
        $this->data = "BEGIN:VCALENDAR\r\nPRODID: Fridahh Eventos www.fridahh.com\r\nVERSION:2.0\r\nMETHOD:PUBLISH\r\nBEGIN:VEVENT\r\nDTSTART:".date("Ymd\THis\Z",strtotime($start))."\r\nDTEND:".date("Ymd\THis\Z",strtotime($end))."\r\nLOCATION:".$location."\r\nTRANSP:OPAQUE\r\nSEQUENCE:0\r\nUID:\r\nDTSTAMP:".date("Ymd\THis\Z")."\r\nSUMMARY:".$name."\r\nDESCRIPTION:".$description."\r\nPRIORITY:1\r\nCLASS:PUBLIC\r\nBEGIN:VALARM\r\nTRIGGER:-PT10080M\r\nACTION:DISPLAY\r\nDESCRIPTION:Reminder\r\nEND:VALARM\r\nEND:VEVENT\r\nEND:VCALENDAR";
    }
    function save() {
		$dir = __DIR__;
        file_put_contents($dir . "/img/eventos/".$this->name.".ics",$this->data);
//		echo $this->name;
    }
    function show() {
...
function isWindowsUser() {
    ...
    // Verifica se a string contém "Windows"
...
function whatsappToHtmlParagraphs($whatsappText) {
    // Substitui as quebras de linha por tags <p>
...
function convertUrlsToLinks($text) {
    ...
    // Substitui cada URL encontrada por um link HTML
    foreach ($matches[0] as $url) {
        $text = str_replace($url, '<a href="' . $url . '" target = "_blank">' . $url . '</a>', $text);
    }

    // Retorna o texto com as URLs convertidas em links HTML
    return $text;
}

function whatsAppToHtmlText($whatsappText) {
    // Substitui os caracteres de formatação do WhatsApp pelas tags HTML correspondentes
    $whatsappText = preg_replace('/\*([^*]+)\*/', '<strong>$1</strong>', $whatsappText);
    $whatsappText = preg_replace('/_([^_]+)_/', '<em>$1</em>', $whatsappText);
    
    // Retorna o texto convertido
    return $whatsappText;
}

function htmlToWhatsAppText($htmlText) {
    // Substitui as tags HTML <strong> e <em> pelas respectivas formatações do WhatsApp
    $htmlText = str_replace(['<strong>', '</strong>', '<em>', '</em>'], ['*', '*', '_', '_'], $htmlText);
...
function confirma($sessao_id,$convidado,$convidado_id){
	global $conexao;
	$q = "SELECT convidado, acao FROM checkins WHERE sessao_id = ".$sessao_id." AND convidado LIKE '".$convidado. "' AND convidado_id=".$convidado_id.";";
...
function slugify($string, $sep="", $limite = 0,$charset = 'utf-8'){
	...
	$string = strtolower(preg_replace('~[\s!*\'();:@&=+$,/?%#[\]]+~', $sep, $string));
	if($limite>0) {
		$string = substr($string,0,$limite);
	}
    return $string;// replace spaces and illegal characters with hyphens
}
...
function formataconfirma($text,$html=1)
{
	if ($text=="1"){
		$text="sim";
		if($html==1){
